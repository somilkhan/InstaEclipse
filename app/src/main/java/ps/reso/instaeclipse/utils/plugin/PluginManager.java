...
public final class PluginManager {
    ...
    public static synchronized void bootstrap(Context context, ClassLoader instagramClassLoader, String instagramVersion) {
        ...
        try {
            ...
            for (PackageInfo info : packages) {
                ...
                try {
                    loadInstalled(context, instagramClassLoader, instagramVersion, info);
                } catch (Throwable error) {
                    ModuleLog.line("(InstaEclipse | Plugin): rejected " + info.packageName + ": " + error.getMessage());
                }
            }
    ...
    public static boolean installFromUri(Context context, ClassLoader ignored, String ignoredInstagramVersion,
        ...
        try {
            ...
            ModuleLog.line("(InstaEclipse | Plugin): installer launch failed: " + error);
            return false;
        }
    }
    ...
    /** Core UI reads its own preferences; injected Instagram reads the same file through LSPosed XSharedPreferences. */
    public static boolean isEnabled(Context context, String id) {
        ...
        try {
            if (CommonUtils.MY_PACKAGE_NAME.equals(context.getPackageName())) {
                return context.getSharedPreferences(PLUGIN_PREFS, Context.MODE_PRIVATE)
                        .getBoolean("enabled_" + id, true);
            }
            XSharedPreferences prefs = new XSharedPreferences(CommonUtils.MY_PACKAGE_NAME, PLUGIN_PREFS);
            prefs.reload();
            return prefs.getBoolean("enabled_" + id, true);
        } catch (Throwable error) {
            ModuleLog.line("(InstaEclipse | Plugin): state read failed for " + id + ": " + error.getMessage());
            return true;
        }
    }
    ...
    public static boolean restartInstagram(Context context) {
        try {
            ...
            if (packageName == null) {
                ModuleLog.line("(InstaEclipse | Plugin): no supported Instagram package installed");
                return false;
            }
            ...
            new Handler(Looper.getMainLooper()).postDelayed(() -> {
                try {
                    ...
                    ModuleLog.line("(InstaEclipse | Plugin): Instagram relaunch failed: " + error);
                }
            }, 1200L);
            return true;
        } catch (Throwable error) {
            ModuleLog.line("(InstaEclipse | Plugin): restart failed: " + error);
            return false;
        }
    }
        ...
        try {
            ...
            for (PackageInfo info : context.getPackageManager().getInstalledPackages(flags)) {
                ...
                if (app != null && app.metaData != null
                        && app.metaData.getBoolean(META_PLUGIN, false)
                        && id.equals(app.metaData.getString(META_PLUGIN_ID))) return info;
            }
        } catch (Throwable ignored) {}
        return null;
    }
    ...
    private static void loadInstalled(Context context, ClassLoader instagramClassLoader,
                                      String instagramVersion, PackageInfo info) throws Exception {
        ApplicationInfo app = info.applicationInfo;
        String id = app.metaData.getString(META_PLUGIN_ID);
        String version = app.metaData.getString(META_PLUGIN_VERSION);
        String entrypoint = app.metaData.getString(META_PLUGIN_ENTRYPOINT);
        if (id == null || version == null || entrypoint == null) throw new SecurityException("plugin metadata incomplete");
        if (!isEnabled(context, id)) {
            ModuleLog.line("(InstaEclipse | Plugin): disabled " + id);
            return;
        }
        if (!signerMatchesCore(context, info)) throw new SecurityException("plugin signer does not match InstaEclipse core");
        PluginManifest manifest = readManifest(app.sourceDir);
        validateManifest(manifest, instagramVersion);
        if (!id.equals(manifest.id) || !entrypoint.equals(manifest.entrypoint))
            throw new SecurityException("plugin metadata/manifest mismatch");
        if (!version.equals(info.versionName) || !version.equals(manifest.version))
            throw new SecurityException("plugin version metadata mismatch");
        ...
        Class<?> entry = Class.forName(entrypoint, true, loader);
        ...
        if (!id.equals(plugin.getId()) || !version.equals(plugin.getVersion()))
            throw new SecurityException("plugin entrypoint identity mismatch");
        try {
            plugin.onLoad(new PluginContext(context, instagramClassLoader, instagramVersion, new PluginLog(id)));
        } catch (Throwable error) {
    ...
    private static void registerRestartReceiver(Context context) {
        ...
        try {
            ...
            ModuleLog.line("(InstaEclipse | Plugin): restart receiver registration failed: " + error);
        }
    }

    private static boolean signerMatchesCore(Context context, PackageInfo plugin) {
        try {
            PackageInfo core = context.getPackageManager().getPackageArchiveInfo(
    ...
    private static boolean sameSigner(SigningInfo a, SigningInfo b) throws Exception {
        Set<String> left = signatureDigests(a), right = signatureDigests(b);
        for (String digest : left) if (right.contains(digest)) return true;
        return false;
    }

    private static Set<String> signatureDigests(SigningInfo info) throws Exception {
    ...
    private static PluginManifest readManifest(String apkPath) throws Exception {
        try (ZipFile zip = new ZipFile(apkPath)) {
            ...
            if (entry == null) throw new SecurityException("plugin manifest missing");
            try (InputStream input = zip.getInputStream(entry)) {
                PluginManifest manifest = GSON.fromJson(
                        new String(readAll(input), StandardCharsets.UTF_8), PluginManifest.class);
                if (manifest == null) throw new SecurityException("plugin manifest invalid");
                return manifest;
            }
        }
    }
    ...
    private static void validateManifest(PluginManifest manifest, String instagramVersion) {
        ...
        if (manifest.id == null || !manifest.id.matches("[a-z0-9][a-z0-9._-]{1,63}"))
            throw new SecurityException("invalid plugin id");
        if (manifest.version == null || !manifest.version.matches("\\d+\\.\\d+\\.\\d+"))
            throw new SecurityException("invalid plugin version");
        if (manifest.entrypoint == null || manifest.entrypoint.length() > 200)
            throw new SecurityException("invalid entrypoint");
        if (manifest.min_core_api > CORE_API || manifest.max_core_api < CORE_API)
            throw new IllegalStateException("core API incompatible");
        long ig = parseLong(instagramVersion);
        if (ig < manifest.min_instagram_version || ig > manifest.max_instagram_version)
            throw new IllegalStateException("Instagram version incompatible");
    }
    ...
    private static void requestPendingTransfer(Context context) {
        try {
            Intent intent = new Intent(ACTION_REQUEST_PENDING).setPackage(CommonUtils.MY_PACKAGE_NAME);
            context.sendBroadcast(intent);
        } catch (Throwable ignored) {}
    }

    private static final class PluginManifest {
        int schema; String id; String name; String description; String version;
        int min_core_api; int max_core_api; String entrypoint;
        long min_instagram_version; long max_instagram_version;
    }
    ...
    private static final class PluginLog implements PluginLogger {
        ...
        @Override public void error(String message, Throwable throwable) {
            ModuleLog.line("(InstaEclipse | Plugin/" + id + "): ERROR " + message + " :: " + throwable);
        }
    }
}
