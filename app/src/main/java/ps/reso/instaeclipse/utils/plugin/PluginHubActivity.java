...
public final class PluginHubActivity extends AppCompatActivity {
    ...
    private boolean restartAfterChange;
    ...
    @Override protected void onResume() {
        super.onResume();
        if (pendingUninstallId != null && !PluginManager.isInstalled(this, pendingUninstallId)) {
    ...
    private void render() {
        ...
        title.setText("Feature Hub"); title.setTextSize(24); title.setTextColor(Color.WHITE);
        header.addView(title, new LinearLayout.LayoutParams(0, -2, 1));
        Button refresh = new Button(this); refresh.setText("Refresh"); refresh.setOnClickListener(v -> loadCatalog(true));
        header.addView(refresh); content.addView(header);

        TextView status = new TextView(this);
        status.setText("Live catalog · v" + catalog.catalogVersion); status.setTextSize(12); status.setTextColor(0xFF8F8F8F); status.setPadding(0, dp(4), 0, dp(8)); content.addView(status);
        ...
        if (catalog.plugins != null) for (PluginCatalogManager.PluginEntry p : catalog.plugins) {
            ...
            any = true; if (PluginManager.isInstalled(this, p.id)) anyInstalled = true; addPlugin(p);
        }
        if (!any) {
            TextView e = new TextView(this); e.setText("No downloadable plugin packs are published yet."); e.setTextSize(15); e.setTextColor(0xFFB7B7B7); content.addView(e);
        }
        if (anyInstalled) addRestartPanel();
    }

    private void addRestartPanel() {
        LinearLayout panel = new LinearLayout(this); panel.setOrientation(LinearLayout.VERTICAL); panel.setPadding(dp(16), dp(14), dp(16), dp(14));
        GradientDrawable bg = new GradientDrawable(); bg.setColor(0xFF171717); bg.setCornerRadius(dp(18)); panel.setBackground(bg);
        TextView message = new TextView(this); message.setText("Plugin changes activate after an Instagram restart."); message.setTextSize(14); message.setTextColor(0xFFB7B7B7); message.setPadding(0, 0, 0, dp(10)); panel.addView(message);
        Button restart = new Button(this); restart.setText("Restart Instagram"); restart.setOnClickListener(v -> {
            if (PluginManager.restartInstagram(this)) Toast.makeText(this, "Restarting Instagram…", Toast.LENGTH_SHORT).show();
            else Toast.makeText(this, "Instagram is not available to restart.", Toast.LENGTH_LONG).show();
        }); panel.addView(restart);
        LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(-1, -2); lp.topMargin = dp(4); lp.bottomMargin = dp(12); content.addView(panel, lp);
    }
    ...
    private void addPlugin(PluginCatalogManager.PluginEntry p) {
        ...
        GradientDrawable bg = new GradientDrawable(); bg.setColor(0xFF171717); bg.setCornerRadius(dp(18)); card.setBackground(bg);
        ...
        if (!installed) { action.setText("Download"); action.setOnClickListener(v -> download(p)); }
            ...
            action.setOnClickListener(v -> {
                ...
                restartAfterChange = true;
                Toast.makeText(this, !enabled ? "Enabled — restarting Instagram…" : "Disabled — restarting Instagram…", Toast.LENGTH_LONG).show();
                if (!PluginManager.restartInstagram(this)) Toast.makeText(this, "Could not restart Instagram automatically.", Toast.LENGTH_LONG).show();
                else restartAfterChange = false;
                render();
            });
        }
        ...
        if (installed) {
            TextView current = new TextView(this); current.setText("Installed v" + installedVersion + (update ? "  ·  Update available" : "  ·  Up to date")); current.setTextSize(12); current.setTextColor(update ? 0xFFFFC107 : 0xFF8F8F8F); current.setPadding(0, dp(8), 0, 0); card.addView(current);
            Button remove = new Button(this); remove.setText("Remove"); remove.setOnClickListener(v -> {
                pendingUninstallId = p.id;
                PluginManager.requestUninstall(this, p.id);
                Toast.makeText(this, "Confirm the Android uninstall prompt.", Toast.LENGTH_LONG).show();
            }); actions.addView(remove);
        }
        if (p.releaseNotes != null && !p.releaseNotes.isEmpty() && update) {
            TextView notes = new TextView(this); notes.setText("What's new: " + p.releaseNotes); notes.setTextSize(13); notes.setTextColor(0xFFB7B7B7); notes.setPadding(0, dp(8), 0, dp(4)); card.addView(notes);
        }
        card.addView(title); card.addView(desc); card.addView(compat); card.addView(actions);
        LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(-1, -2); lp.bottomMargin = dp(12); content.addView(card, lp);
    }
