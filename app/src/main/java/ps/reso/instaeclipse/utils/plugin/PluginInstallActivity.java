...
public final class PluginInstallActivity extends Activity {
    ...
    private void prepare(Uri uri, String expectedSha) {
        try {
            ...
            try (InputStream input = getContentResolver().openInputStream(uri); FileOutputStream output = new FileOutputStream(stagedApk)) {
            ...
            if (candidate.packageName == null || !candidate.packageName.startsWith("ps.reso.instaeclipse.plugins.")) throw new SecurityException("invalid plugin package name");
            if (expectedPackageName != null && !expectedPackageName.isEmpty() && !expectedPackageName.equals(candidate.packageName)) throw new SecurityException("plugin package name mismatch");
            if (!manifest.version.equals(candidate.versionName)) throw new SecurityException("plugin APK version mismatch");
    ...
    @Override protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK) {
            broadcastInstalled();
            Toast.makeText(this, "Plugin installed. Restarting Instagram…", Toast.LENGTH_LONG).show();
            new android.os.Handler(getMainLooper()).postDelayed(() -> {
                PluginManager.restartInstagram(this);
                cleanupStagedApk();
                finish();
            }, 250L);
            return;
    ...
    private void broadcastInstalled() {
        ...
        Intent done = new Intent(PluginManager.ACTION_PLUGIN_INSTALLED).setPackage(getPackageName()).putExtra(PluginManager.EXTRA_ID, expectedId).putExtra(PluginManager.EXTRA_VERSION, expectedVersion);
        sendBroadcast(done);
    }
    private void cleanupStagedApk() { if (stagedApk != null) stagedApk.delete(); }
    private void fail(String message) { cleanupStagedApk(); Toast.makeText(this, message, Toast.LENGTH_LONG).show(); finish(); }
