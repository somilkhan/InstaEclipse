...
public class LoggingFragment extends Fragment {
    ...
    private final BroadcastReceiver logReplyReceiver = new BroadcastReceiver() {
        @Override public void onReceive(Context context, Intent intent) {
            ...
            int gen = loadGeneration.get(); cancelInstagramTimeout();
            loadExecutor.execute(() -> {
                String instagram = formatInstagramSection(intent);
                String combined = joinSections(instagram, companionSection);
                mainHandler.post(() -> { if(contentView!=null&&gen==loadGeneration.get()) showCombined(combined); });
            });
        }
    };
    ...
    @Override public void onDestroyView(){super.onDestroyView();cancelInstagramTimeout();contentView=null;lineCountView=null;}
    @Override public void onDestroy(){super.onDestroy();loadExecutor.shutdownNow();}

    private void loadLogs(){Context ctx=getContext();if(ctx==null||contentView==null)return;int gen=loadGeneration.incrementAndGet();cancelInstagramTimeout();contentView.setText(R.string.logging_placeholder);loadExecutor.execute(()->{String companion=formatCompanionSection(ctx);String pkg=findInstagramPackage(ctx);mainHandler.post(()->{if(contentView==null||gen!=loadGeneration.get())return;companionSection=companion;if(pkg==null){showCombined(joinSections(formatInstagramUnavailableSection(),companionSection));return;}Intent request=new Intent(CommonUtils.ACTION_REQUEST_LOGS);request.setPackage(pkg);ctx.sendBroadcast(request);scheduleInstagramTimeout(gen);});});}
    private void scheduleInstagramTimeout(int gen){pendingTimeout=()->{if(contentView==null||gen!=loadGeneration.get())return;String ig=sectionHeader(getString(R.string.logging_instagram),null)+getString(R.string.logging_instagram_timeout)+"\n";showCombined(joinSections(ig,companionSection));};mainHandler.postDelayed(pendingTimeout,INSTAGRAM_REPLY_TIMEOUT_MS);}
    private void cancelInstagramTimeout(){if(pendingTimeout!=null){mainHandler.removeCallbacks(pendingTimeout);pendingTimeout=null;}}
    private String formatCompanionSection(Context ctx){String snap=Logging.getSnapshot().trim();if(snap.isEmpty())return "";return sectionHeader(getString(R.string.logging_companion),ctx.getPackageName())+snap;}
    private static String joinSections(String a,String b){if(b==null||b.isEmpty())return a;if(a==null||a.isEmpty())return b;return a+"\n\n"+b;}
    private String formatInstagramUnavailableSection(){return sectionHeader(getString(R.string.logging_instagram),null)+getString(R.string.logging_no_target);}
    private String formatInstagramSection(Intent intent){String src=intent.getStringExtra(CommonUtils.EXTRA_LOG_SOURCE);String header=sectionHeader(getString(R.string.logging_instagram),src);String err=intent.getStringExtra(CommonUtils.EXTRA_LOG_ERROR);if(err!=null)return header+err;String body=intent.getStringExtra(CommonUtils.EXTRA_LOG_TEXT);if(body==null||body.trim().isEmpty())return header+getString(R.string.logging_empty_reply);if(body.length()>MAX_DISPLAY_CHARS)body=body.substring(0,MAX_DISPLAY_CHARS)+"\n\n[Truncated - logs too large to display]";return header+body.trim();}
    private static String sectionHeader(String title,String pkg){String h="=== "+title+" ===\n";if(pkg!=null)h+="["+pkg+"]\n";return h+"\n";}
    private void showCombined(String text){if(contentView==null)return;contentView.setText(text);if(lineCountView==null)return;int lines=1;for(int i=0;i<text.length();i++)if(text.charAt(i)=='\n')lines++;lineCountView.setVisibility(View.VISIBLE);lineCountView.setText(getString(R.string.logging_lines_format,lines));}
